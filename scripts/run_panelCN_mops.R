...
# modified function to select/add gene regions to plot
makeGenesDataFromTxDb.mod <- function(txdb, karyoplot, genes = NULL) {
	...
	if (!is.null(genes)) {
		all.genes <- subset(all.genes, gene_id %in% genes);
		}

	if (!is.null(karyoplot)) {
	...
	res <- addGeneNames(res);
	res <- mergeTranscripts(res);
...
if (make_pon) {
	...
	for (i in 1:length(sampleBams)) {
		...
		gr_ob$Gene <- results.table[[i]]$Gene;
		gr_ob$baf <- 1;
		...
		# output plots for each gene of interest
		genes.of.interest <- unlist(strsplit('MLH1|MSH2|MSH3|MSH6|PMS2|APC|POLD1|POLE|MUTYH|TP53|BRAF|BRCA1|BRCA2','\\|'));

		output.file <- paste0(sampleNames[i], "_panelcn.mops_per-gene_plots.pdf");
		...
		for (gene in genes.of.interest) {
			...
			# organize all transcripts information
			...
			kpPlotGenes(kp, data = transcript.data, data.panel = 2, add.gene.names = FALSE,
				r0 = if (length(transcript.data$genes) == 1) { 0.75 } else { 0.7 },
				r1 = if (length(transcript.data$genes) == 1) { 0.9 } else { 1 } );
			kpAddLabels(kp, cex = 1, labels = gene, data.panel = 2,
				r0 = if (length(transcript.data$genes) == 1) { 0.75 } else { 0.7 },
				r1 = if (length(transcript.data$genes) == 1) { 0.9 } else { 1 } );

			# add LRR to top of plot
			plotLRR(kp, cnplot_dat, ymin = -1.5, ymax = 1.5, labels = NA, data.panel = 1,
				line.at.0 = TRUE, line.at.0.col = "black", points.col = "dodgerblue2",
				points.cex = 0.8, add.axis = FALSE, r0 = 0.2, r1 = 1);

			kpAxis(kp, ymin = -1.5, ymax = 1.5, col = "gray50", cex = 1, numticks = 5, r0 = 0.2, r1 = 1);
			kpAddLabels(kp, labels = expression('log'['2']*'(Depth Ratio)'), srt = 90, 
				cex = 1, pos = 3, label.margin = 0.07, r0 = 0.2, r1 = 1);

			# add CN to bottom of plot
			plotCopyNumberCallsAsLines(kp, cn.calls = cnplot_dat, col = "red", lwd = 3,
				style = 'line', labels = NA, ymin = 0, ymax = 4, add.axis = FALSE,
				data.panel = 2, r0 = 0, r1 = 0.5);

			kpAxis(kp, ymin = 0, ymax = 4, col = "gray50", cex = 1, numticks = 5, data.panel = 2, r0 = 0, r1 = 0.5);
			kpAddLabels(kp, labels = "Copy-Number Estimate", srt = 90, cex = 1, pos = 3, 
				label.margin = 0.07, data.panel = 2, r0 = 0, r1 = 0.5);
			}
