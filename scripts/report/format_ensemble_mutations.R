...
tool.count <- sum(run.mutect2,run.pindel,run.somaticsniper,run.vardict);
snp.tool.count <- sum(run.mutect2,run.somaticsniper,run.vardict);
indel.tool.count <- sum(run.mutect2,run.pindel,run.vardict);
...
link.name <- paste0(arguments$project, '_ensemble_mutation_data.tsv');
unlink(link.name);
