	...
	if ($args{step3}) {
		...
		if ('Y' eq $tool_set{'ichor_cna'}) {
			...
			if ($args{dry_run}) {
				...
				push @step3_job_ids, $ichor_run_id;
				push @job_ids, $ichor_run_id;
				}
			}

		## run GATK's newer MuTect2 pipeline
...
#				}
		...
		if ('Y' eq $tool_set{'novobreak'}) {
			...
			if ($args{dry_run}) {
				...
				push @step4_job_ids, $novobreak_run_id;
		...
		if (scalar(@step4_job_ids) > 3) {
			$current_dependencies = join(':', @step4_job_ids);
			} elsif (scalar(@step3_job_ids) > 3) {
			$current_dependencies = join(':', @step3_job_ids);
			}
		...
		if ('Y' eq $tool_set{'mavis'}) {
			...
			# (Delly/NovoBreak/Pindel resuts), this can only be run AFTER these
